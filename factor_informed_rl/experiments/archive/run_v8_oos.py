"""V8 OOS: Select@2021-01-01 → V7 models → 2021~2026 (No look-ahead)"""
...
SEL_DATE = pd.Timestamp("2021-01-04")  # 2021年第一个交易日
WINDOW = 60
...
# ── Selection @ 2021-01-04 ──
print(f"Selection @ {SEL_DATE.date()} (PIT-safe: only data ≤ {SEL_DATE.date()})")
stock_info = {}
...
for f in os.listdir(CACHE):
    ...
    # 只用 ≤ SEL_DATE 的数据 (PIT-safe)
    df = df[df.index <= SEL_DATE]
...
print(f"  {len(stock_info)} stocks indexed")
...
for sn, cfg in STRATEGIES.items():
    ...
    print(f"  [{sn}]: {len(vc)}→6: {selections[sn][:4]}...")

# Use Value's picks for fair comparison
TEST_CODES = selections["Value-Defensive"]
print(f"\n  Trading pool: {TEST_CODES}")

# ── Backtest 2021-2026 ──
print(f"\nBacktest: 2021-01-01 → 2026-06-30\n")
...
for code in TEST_CODES:
    ...
    for sn, cfg in STRATEGIES.items():
        ...
        if 'pe_percentile' in cfg["factors"]:
            tmp_df['pe_percentile'] = tmp_df['pe'].expanding(min_periods=60).rank(pct=True).fillna(0.5)
        ...
        env = TradingEnv(test_tmp, engine, sb, Denoiser(method="none"),
...
csi_bh = float(csi_t['close'].iloc[-1]/csi_t['close'].iloc[0]-1) if len(csi_t)>1 else 0
csi_sr = float(csi_t['close'].pct_change().dropna().mean()/
              (csi_t['close'].pct_change().dropna().std()+1e-10)*np.sqrt(252)) if len(csi_t)>1 else 0

print(f"{'='*65}")
print(f"  CSI300 (2021-2026): {csi_bh:+.1%} Sharpe={csi_sr:.2f}")
print(f"  Trading pool: {TEST_CODES}")
print(f"{'='*65}")

for sn in ["Value-Defensive","Quality-Offensive","Bare PPO"]:
    sub = df_r[df_r['strategy']==sn]
    if len(sub)==0: continue
    print(f"\n  [{sn}] ({len(sub)} stocks, 5.5y test)")
