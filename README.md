...
1. **因子软约束**：在 PPO 目标函数中增加两项惩罚
   ...
   - **正交性项** `L_ortho`：约束动作不与已知因子正交（充分利用因子信息）
...
这一设计借鉴了 Physics-Informed Neural Networks (PINN) 的思路，将领域知识（金融因子）作为软约束注入深度学习模型。
...
`models/factor_loss.py` 实现了三类损失：

| 损失 | 公式 | 含义 |
|------|------|------|
| IC 保持 | `mean(ReLU(-IC - τ)²)` | 动作与因子方向一致（τ 为阈值） |
| 正交性 | `mean(ReLU(|corr| - ρ)²)` | 动作与因子相关但不过度依赖 |
...
`portfolio/manager.py` 状态机：

```
WATCHING → BUILDING → ACTIVE → EXITING → DONE
```
...
- **12 股票交叉验证**：验证模型泛化能力，避免单票过拟合
- **样本外测试**：训练集 2015-2020，测试集 2021+
- **未来函数检测**：PE 分位用 expanding rank（PIT-safe）
- **开盘价执行**：避免收盘价执行的夏普虚高
